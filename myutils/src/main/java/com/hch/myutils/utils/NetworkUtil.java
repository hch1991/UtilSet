...
public class NetworkUtil {
    ...
    //连接指定wifi
    public static void connectWifi(Context mContext, WifiManager wifiManager, String ssid, String password, int wifiType) {
        Log.d("Cache_Log", "ssid: " + ssid + "password: " + password + "wifiType: " + wifiType);
        int netId = wifiManager.addNetwork(createWifiConfig(mContext,ssid, password, wifiType));
    ...
    //根据wifi加密类型配置wifi设置
    public static WifiConfiguration createWifiConfig(Context mContext,String SSID, String password, int type) {
    ...
    //获取本地ip
    public static String getIPAddress(Context mContext, WifiManager wifiManager) {
        NetworkInfo info = ((ConnectivityManager) mContext
                .getSystemService(Context.CONNECTIVITY_SERVICE)).getActiveNetworkInfo();
        if (info != null && info.isConnected()) {
            if (info.getType() == ConnectivityManager.TYPE_MOBILE) {//当前使用2G/3G/4G网络
                MLog.d("-------------------Mobile network");
                try {
